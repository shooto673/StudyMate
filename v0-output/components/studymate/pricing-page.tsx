  ...
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <main className="mx-auto max-w-6xl px-4 py-12">
        ...
        >
          <h1 className="mb-4 text-3xl font-bold text-[var(--text)] md:text-4xl">
            料金プラン
          </h1>
          ...
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`relative rounded-3xl bg-white p-6 shadow-[var(--card-shadow)] transition-all ${
                plan.highlight
                  ? "scale-105 ring-2 ring-[var(--primary)]"
                  : plan.id === "premium"
            ...
            >
              ...
              {plan.badge && (
                <div
                  className={`absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 rounded-full px-4 py-1 text-xs font-bold text-white ${
                    plan.id === "premium" ? "bg-[var(--warning)]" : "bg-[var(--primary)]"
                  }`}
                >
                  {plan.id === "premium" ? (
              ...
              <div className="mb-4 flex justify-center">
                <span className="rounded-full bg-[var(--bg-sub)] px-3 py-1 text-xs font-medium text-[var(--text-sub)]">
                  英語・数学 全教科対応
                </span>
              </div>
              ...
              <motion.button
                ...
                className={`w-full rounded-2xl py-3 font-bold transition-all ${
                  plan.id === "free"
                    ? "cursor-not-allowed bg-[var(--bg-sub)] text-[var(--text-muted)]"
                    : plan.highlight
                    ? "bg-[var(--primary)] text-white shadow-md hover:shadow-lg"
                    : plan.id === "premium"
                    ? "bg-[var(--warning)] text-white shadow-md hover:shadow-lg"
        ...
        >
          <p className="text-sm text-[var(--text-muted)]">
            ...
            <br />
            ご不明な点は
            <button className="text-[var(--primary)] underline hover:no-underline">
              お問い合わせ
            </button>
            ください。
          </p>
