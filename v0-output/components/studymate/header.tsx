...
export function Header({ isLoggedIn = false, grade, onNavigate, className }: HeaderProps) {
  return (
    <motion.header
      initial={{ y: -20, opacity: 0 }}
      animate={{ y: 0, opacity: 1 }}
      className={cn(
        "sticky top-0 z-50 w-full border-b border-[var(--card-border)] bg-white/80 backdrop-blur-md",
        className
      )}
    >
