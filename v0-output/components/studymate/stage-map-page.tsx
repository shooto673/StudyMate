...
export function StageMapPage({
  ...
  const [activeSubject, setActiveSubject] = useState<"english" | "math">("english")
  ...
  const currentUnitIndex = filteredUnits.findIndex((u) => u.progress > 0 && u.progress < 80)
  ...
  const getNodeStyle = (unit: Unit) => {
    if (unit.progress >= 80) return "mastered"
    if (unit.progress > 0) return "learning"
    return "unlocked"
  }

  const subjectColor = activeSubject === "english" ? "var(--english)" : "var(--math)"
  const subjectColorLight = activeSubject === "english" ? "var(--english-light)" : "var(--math-light)"
  const subjectColorHex = activeSubject === "english" ? "#4DABF7" : "#FF922B"
  const subjectColorLightHex = activeSubject === "english" ? "#E7F5FF" : "#FFF4E6"

  // Calculate zigzag positions for Duolingo-style path
    ...
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <main className="mx-auto max-w-lg px-4 py-6">
        ...
        <motion.div
          ...
          className="mb-6 rounded-3xl bg-white p-5 shadow-[var(--card-shadow)]"
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-xs">
                <span className="rounded-full bg-[var(--primary-light)] px-3 py-1 text-[var(--primary)] font-medium">
                  {stats.totalAnswers}問回答
                </span>
                <span className="rounded-full bg-[var(--success-light)] px-3 py-1 text-[var(--success)] font-medium">
                  {stats.accuracy}%正答率
                </span>
                <span className="rounded-full bg-[var(--accent-light)] px-3 py-1 text-[var(--accent)] font-medium">
                  {stats.streak}日連続
                </span>
              </div>

              <div className="mt-2 text-xs text-[var(--text-sub)]">
                今日 {usageToday}/{dailyLimit}問
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="mb-6 flex justify-center gap-2">
          <motion.button
            ...
            className={`flex items-center gap-2 rounded-2xl px-6 py-3 font-bold transition-all ${
              activeSubject === "english"
                ? "bg-[var(--english)] text-white shadow-md"
                : "bg-[var(--english-light)] text-[var(--english)]"
            }`}
          >
          ...
          <motion.button
            ...
            className={`flex items-center gap-2 rounded-2xl px-6 py-3 font-bold transition-all ${
              activeSubject === "math"
                ? "bg-[var(--math)] text-white shadow-md"
                : "bg-[var(--math-light)] text-[var(--math)]"
            }`}
          >
            ...
            >
              ...
              >
                    ...
                    <path
                      ...
                      d={`M ${startX} ${startY} 
                          C ${startX} ${startY + 60}, 
                            ${endX} ${endY - 60}, 
                            ${endX} ${endY}`}
                      stroke={isCompleted ? subjectColorHex : "#DEE2E6"}
                ...
                {filteredUnits.length > 0 && (
                  <path
                    d={`M ${100 + getNodePosition(filteredUnits.length - 1)} ${(filteredUnits.length - 1) * 160 + 50} 
                        C ${100 + getNodePosition(filteredUnits.length - 1)} ${(filteredUnits.length - 1) * 160 + 110}, 
                          100 ${filteredUnits.length * 160 - 10}, 
                          100 ${filteredUnits.length * 160 + 50}`}
                    stroke={allCompleted ? "#FCC419" : "#DEE2E6"}
                    strokeWidth={allCompleted ? 6 : 4}
                    strokeLinecap="round"
                    strokeDasharray={allCompleted ? "none" : "12 8"}
                    fill="none"
              ...
              <div className="relative z-10 flex flex-col items-center">
                  ...
                  return (
                    ...
                    >
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() => onSelectUnit(activeSubject, unit.slug)}
                      ...
                      >
                        ...
                        <div className="relative">
                          {/* Progress Ring for learning state */}
                          {style === "learning" && (
                            <svg
                            ...
                            >
                              ...
                              <motion.circle
                                ...
                                r="44"
                                fill="none"
                                stroke={subjectColorHex}
                                strokeWidth="8"
                                strokeLinecap="round"
                                initial={{ strokeDasharray: "0 277" }}
                                animate={{ strokeDasharray: `${(unit.progress / 100) * 277} 277` }}
                          ...
                          <div
                            ...
                            style={{
                              backgroundColor: style === "mastered" ? subjectColorHex : "#FFFFFF",
                              borderColor: style === "unlocked" ? "#DEE2E6" : undefined,
                              boxShadow: style === "mastered" 
                                ? `0 4px 20px ${subjectColorHex}40`
                                : style === "learning"
                                ? `0 4px 20px ${subjectColorHex}30`
                          ...
                          >
                            {style === "mastered" ? (
                              ...
                              <FileText 
                                className={`h-8 w-8 ${
                                  style === "learning" ? "" : "text-[#ADB5BD]"
                                }`}
                                style={{ color: style === "learning" ? subjectColorHex : undefined }}
                              />
                            )}
                          </div>

                          {/* Progress Badge for learning */}
                          {style === "learning" && (
                            <motion.div
                              initial={{ scale: 0 }}
                              animate={{ scale: 1 }}
                              transition={{ delay: index * 0.1 + 0.4, type: "spring" }}
                              className="absolute -bottom-3 left-1/2 -translate-x-1/2 rounded-lg px-3 py-1 text-xs font-bold text-white shadow-md"
                        ...
                        {/* Mascot cheering for current unit */}
                        {isCurrent && (
                          <motion.div
                            initial={{ opacity: 0, x: xOffset > 0 ? -30 : 30 }}
                ...
                <motion.div
                  initial={{ opacity: 0, scale: 0 }}
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ delay: filteredUnits.length * 0.1, type: "spring" }}
                  style={{ height: 160 }}
                  className="flex items-start justify-center pt-4"
                >
                  <div
                  ...
                  >
                    <div
                      className={`flex h-24 w-24 items-center justify-center rounded-full ${
                        allCompleted
                          ? "bg-gradient-to-br from-[#FCC419] to-[#FF922B] shadow-xl"
                          : "border-4 border-dashed border-[#DEE2E6] bg-white"
                      }`}
                      style={{
                        boxShadow: allCompleted ? "0 8px 30px rgba(252, 196, 25, 0.4)" : undefined,
                      }}
                    >
                  ...
                  {allCompleted && (
                    <motion.div 
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.5 }}
                      className="absolute mt-32 flex justify-center gap-4"
                    >
